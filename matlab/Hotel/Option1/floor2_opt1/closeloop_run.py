...
b=np.array([[tab2['b2'].iloc[0],tab2['b27'].iloc[0]]])
x0=np.array([initial['t2'].iloc[0],initial['t27'].iloc[0]])
for i in range(1,len(tab2)):
     temp=np.array([[tab2['b2'].iloc[i],tab2['b27'].iloc[i]]])
     #print temp
     b=np.concatenate((b, temp))
...
x=np.arange(len(tab['t2']))   

plt.plot(x,tab['t2'],label='Zone2 predicted',color='b')   
plt.plot(x,initial['t2'],label='Zone2 real',color='r')   
...
plt.clf()
plt.plot(x,tab['t27'],label='Zone27 predicted',color='b')   
plt.plot(x,initial['t27'],label='Zone27 real',color='r')   
#plt.plot(x,tab2['t1'],label='real',color='r')   
plt.legend()
plt.xlabel('Timestep',fontsize=10)
plt.ylabel('Predicted Temperature [degC]',fontsize=10) 
plt.savefig('zone27timestep_cl.png',bbox_inches = 'tight',pad_inches = 0)     
plt.show()


x=np.arange(20,initial['t2'].max()+1)

plt.scatter(initial['t2'],tab['t2'],label='Zone2 close loop RMSE:'+str(round(rmse(initial['t2'],tab['t2']), 3))+')',color='y')
