...
if Flag_SingleRun
    X = 22*ones(15,1);  % '15' stands for the dimension of the state space model
    InitialStates.X = X;
...
c_e_perday(N_sch*2/3:N_sch*5/6) = 0.1*1e-3;
...
Zoneparam.Ts = f2c(55)*ones(15,1); % supply air temperature
Zoneparam.T_approx = 22*ones(15,1); % approximation for room temperature for complexity
...
T_high = 26*ones(15,1); % temperature bounds
T_low = 20*ones(15,1);
Tref_high = 26*ones(15,1); % temperature set-points bounds
Tref_low = 18*ones(15,1);
...
m_min2= [5.09444e-002;0.12029;0.19939;0.11254;2.45639;0.76339;0.65163];
m_max2= [0.10189;0.24058;0.39879; 0.22508;4.91277;1.52677;3.25816];

m_min3=0;
m_max3=0.88275+0.86971+4.40470+5.57902+0.79506+0.80446;

m_min4=0.38311;
m_max4=1.91553;

m_min5=0;
m_max5=0.25859+0.24952+1.05050;

m_min6=[1.57519;0.18780;1.62522;1.43589];
m_max6=[2.25027;0.88596;2.32175;2.05127];

m_min = [m_min1;m_min2;m_min3;m_min4;m_min5;m_min6]; % supply air flow rate limits (eio file)
%m_max =[m_max1;2*m_max2;7*m_max3;7*m_max4;2*m_max5;2*m_max6]; % working bounds
m_max =[m_max1;1*m_max2;6*m_max3;6*m_max4;1*m_max5;1*m_max6];
...
Option3ModelTest
%load extvar_option3_hotel.mat  % loads disturbance data needed for the zone model
...
variables u(N_sch,5) m_s(N_sch,15) Prh(N_sch,15) T_ref(N_sch,15)
% u =[Pb_in,Pb_out,Pfc_f_ref,Pfc_h_ref,Ras]
% m_s: Supply Mass flow rate from AHU

% State variables:
% zone temperature, battery SOC
variables x(N_sch,15,N_perhour) Eb(N_sch,1,N_perhour)  % Three dimensional vector (second column represents 'size')
%('15' includes zone(15))
...
for i_sch = 1:N_sch  % (nS=24)
    ...
    for i_state = 1:N_perhour
        ...
        Xnew=A*X+B*U+E*D; % X=[Basement,Bot_floor_zones,Mid_floor_guest,Mid_floor_corridor,Top_floor_guest,Top_floor_zones] total = 15 states 
        ...
        T_low + kas*Ras_i/3 <= T_i <= T_high - kas*Ras_i/3;
        0 <= Eb_i <= Eb_max;
...
%         plot(tvec_sch(2:end),10e3*c_e)
    ...
    plot(tvec_sch(1:end-1),sum(Prh,2)+Ppre_total)
