...
j=0
for key2 in info["inputs"]["zones"]:
    j=j+1
    tab['t'+str(j)]=[0]*len(tab)
    temp=[]
    area=0
    for key in info["inputs"]["zones"][key2]:
          tab['t'+str(j)]=tab['t'+str(j)]+(tab[info["inputs"]["zones"][key2][key].upper()+
             ':Surface Inside Face Temperature [C](TimeStep)']+tab[info["inputs"]["zones"][key2][key].upper()+
                                               ':Surface Outside Face Temperature [C](TimeStep)'])/2*(info["inputs"]["wallarea"][key2][key])
          area=area+info["inputs"]["wallarea"][key2][key]
    tab['w'+str(j)]=tab['w'+str(j)]/area                            
...
for i in range(len(zones)):
    ...
    mrh.append(str(zones[i]).upper()+' VAV BOX REHEAT COILDEMAND OUTLET NODE:System Node Mass Flow Rate [kg/s](TimeStep)')
