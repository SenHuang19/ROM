...
for i in range(len(tab)-1):
    ...
    z.append(tab['sp0'][i:i+1])
    z.append(tab['m'][i:i+1])
    z.append(tab['rh'][i:i+1])
    """
...
# temperature along time plotting      
x=np.arange(len(tab2['real']))   
  
plt.plot(x,tab2['real'],color='r')   
plt.plot(x,tab2['prediction'],color='b') 
plt.legend()
plt.xlabel('Timestep',fontsize=10)
plt.ylabel('Prediction [degC]',fontsize=10)    
plt.savefig(filename+'timestep_ol.png',bbox_inches = 'tight',pad_inches = 0)  
plt.show()

"""
# train data plotting
tab2=pd.read_csv(filename+'train.csv')
x=np.arange(len(tab2['t1']))   
plt.plot(x,tab2['t1'],color='b')   
plt.xlabel('Timestep',fontsize=10)
plt.ylabel('Temperature [degC]',fontsize=10)   
plt.show()


# validation data plotting
tab2=pd.read_csv(filename+'validation.csv')
x=np.arange(len(tab2['t1']))   
plt.plot(x,tab2['t1'],color='r')   
plt.xlabel('Timestep',fontsize=10)
plt.ylabel('Temperature [degC]',fontsize=10)   
plt.show()
"""
