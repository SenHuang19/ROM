...
b=np.array([[tab2['b4'].iloc[0],tab2['b5'].iloc[0],tab2['b6'].iloc[0]]])
x0=np.array([initial['t4'].iloc[0],initial['t5'].iloc[0],initial['t6'].iloc[0]])
for i in range(1,len(tab2)):
     temp=np.array([[tab2['b4'].iloc[i],tab2['b5'].iloc[i],tab2['b6'].iloc[i]]])
     #print temp
     b=np.concatenate((b, temp))
